...
interface Star {
  ...
  delay: number;
  duration: number;
  trajectory: number; // Random horizontal movement
}
...
export default function ClickStars() {
  ...
  useEffect(() => {
    const handleClick = (e: MouseEvent) => {
      const starCount = Math.floor(Math.random() * 5) + 2; // 2-6 stars
      const newStars: Star[] = [];
      ...
      for (let i = 0; i < starCount; i++) {
        newStars.push({
          ...
          duration: Math.random() * 400 + 600, // 600-1000ms duration
          trajectory: (Math.random() - 0.5) * 100, // -50 to +50px horizontal movement
        });
      }
      ...
      }, 1200); // Slightly longer than max duration
    };
